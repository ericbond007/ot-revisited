// fort-bridger-art.jsx
// Fort Bridger (Jim Bridger's post, 1843+ era).
...
function FortBridgerArt() {
  const F = "fb";
  const ink = LMK.ink;
  const log = "#8a5a30";          // log wall
  const logSh = "#5a3818";
  const logHi = "#a87a48";
  const sodRoof = "#7a6638";      // dark dirt/sod roof
  const sodRoofSh = "#4a3818";
  const sodGrass = "#7a8458";     // grass on top of sod
  ...
  return (
    <g>
      ...
      <g>
        ...
        />
        {/* snow caps */}
        <g fill={LMK.white} opacity="0.85" stroke={ink} strokeWidth="0.25">
      ...
      <g>
        ...
        <g>
          {Array.from({ length: 32 }, (_, i) => {
            ...
            return (
              <path key={i}
                    d={`M ${tx} ${treelineY} L ${tx + 1.5} ${treelineY - th} L ${tx + 3} ${treelineY} Z`}
      ...
      <g>
        ...
        <path d="M 30 158 L 70 156 M 200 156 L 240 154 M 340 152 L 380 150"
              stroke={LMK.white} strokeWidth="0.4" opacity="0.55" />
      </g>
      ...
      <g>
        ...
        {Array.from({ length: 28 }, (_, i) => {
          ...
          const py = 122 + (i / 27) * (-2) + 138;
          // posts run along the back of the cabin compound
          return (
            <path key={i}
                  d={`M ${px} 138 L ${px} 122 L ${px + 1.3} 119 L ${px + 2.6} 122 L ${px + 2.6} 138 Z`}
                  fill={stockade} stroke={ink} strokeWidth="0.3" />
          );
        })}
      </g>
      ...
      <g>
        ...
        <g>
          ...
          <g fill={logHi} stroke={ink} strokeWidth="0.3">
          ...
          <path d="M 108 122 L 130 113 L 152 122 L 150 122 L 130 114.5 L 110 122 Z"
                fill={sodRoof} stroke={ink} strokeWidth="0.5" />
          <path d="M 108 122 L 130 113 L 152 122 L 152 124 L 130 115 L 108 124 Z"
                fill={sodRoofSh} stroke={ink} strokeWidth="0.45" />
          {/* grass tufts on roof */}
          <g stroke={sodGrass} strokeWidth="0.6" fill="none">
          ...
          {/* heavy plank door */}
          <rect x="125" y="128" width="6" height="10" fill={LMK.earthDark} stroke={ink} strokeWidth="0.4" />
          <line x1="127" y1="128" x2="127" y2="138" stroke={ink} strokeWidth="0.25" />
          <line x1="129" y1="128" x2="129" y2="138" stroke={ink} strokeWidth="0.25" />
          {/* tiny window */}
          <rect x="138" y="128" width="4" height="4" fill={LMK.inkSoft} stroke={ink} strokeWidth="0.3" />
          {/* chimney + smoke (forge) */}
          <rect x="143" y="111" width="3.5" height="6" fill={LMK.earthDark} stroke={ink} strokeWidth="0.3" />
        ...
        <g>
          ...
          <path d="M 156 120 L 175 113 L 215 114 L 215 116 L 175 115 L 158 121 Z"
                fill={sodRoof} stroke={ink} strokeWidth="0.45" />
          {/* grass tufts */}
          <g stroke={sodGrass} strokeWidth="0.55" fill="none">
            <path d="M 180 116 q 0.5 -2 1 0" />
            <path d="M 192 115 q 0.5 -2 1 0" />
            <path d="M 204 115 q 0.5 -2 1 0" />
          </g>
          {/* door + window */}
          <rect x="184" y="124" width="5" height="10" fill={LMK.earthDark} stroke={ink} strokeWidth="0.3" />
          <rect x="198" y="124" width="4" height="4" fill={LMK.inkSoft} stroke={ink} strokeWidth="0.3" />
        </g>

        {/* trade-room hand-painted sign hanging from cabin A eave */}
        <g>
          <line x1="148" y1="124" x2="148" y2="129" stroke={ink} strokeWidth="0.4" />
          <rect x="148" y="129" width="9" height="4" fill="#cca870" stroke={ink} strokeWidth="0.35" />
          <line x1="150" y1="131" x2="155" y2="131" stroke={ink} strokeWidth="0.4" opacity="0.7" />
        </g>
        ...
        <g transform="translate(96 138)">
          ...
          <path d="M -7 -1 q -1 -3 0.5 -5 q 1.5 2 -0.5 5"
                fill="#f0c060" stroke={ink} strokeWidth="0.25" opacity="0.85" />
        </g>
      </g>
      ...
      <g>
        <path d="M 250 138 L 250 128 L 318 128 L 318 138"
              fill="none" stroke={stockade} strokeWidth="0.6" />
        {/* posts */}
        {[250, 264, 278, 292, 306, 318].map(px => (
          <line key={px} x1={px} y1="138" x2={px} y2="126" stroke={stockade} strokeWidth="0.7" />
        ))}
        {/* rails */}
        <line x1="250" y1="131" x2="318" y2="131" stroke={stockade} strokeWidth="0.5" />
        <line x1="250" y1="135" x2="318" y2="135" stroke={stockade} strokeWidth="0.5" />
        {/* 2 horses in the pen */}
        <Horse x={266} y={138} ink={ink} color="#5a3a20" />
        <Horse x={296} y={138} ink={ink} color="#3a2818" />
      </g>

      {/* ── native tipi camp — right side of post ── */}
      <g>
        <Tipi cx={350} cy={138} h={26} ink={ink} fill={tipi} sh={tipiSh} />
        <Tipi cx={385} cy={138} h={22} ink={ink} fill={tipi} sh={tipiSh} smoke />
        <Tipi cx={414} cy={138} h={28} ink={ink} fill={tipi} sh={tipiSh} />
        {/* small figure + dog */}
        <g>
          <rect x="370" y="134" width="1.4" height="4" fill={ink} />
          <circle cx="370.7" cy="133" r="0.9" fill={LMK.earthLight} stroke={ink} strokeWidth="0.2" />
          {/* dog */}
          <ellipse cx="378" cy="137" rx="2" ry="0.9" fill={LMK.earthDark} />
          <line x1="376" y1="137" x2="376" y2="138" stroke={ink} strokeWidth="0.4" />
          <line x1="380" y1="137" x2="380" y2="138" stroke={ink} strokeWidth="0.4" />
        </g>
      </g>

      {/* ── sage brush in foreground ── */}
      <g>
        {[15, 50, 200, 330, 440].map((sx, i) => (
          <SageBrush key={i} x={sx} y={172} ink={ink} />
        ))}
      </g>
...
// ── Tipi — conical with poles protruding from the top ──
function Tipi({ cx, cy, h = 24, ink, fill, sh, smoke = false }) {
  const w = h * 0.7;
  return (
    <g>
      {/* cone */}
      <path
        ...
        fill={fill} stroke={ink} strokeWidth="0.55"
      />
      {/* shadow side */}
      <path
        d={`M ${cx - w / 2} ${cy} L ${cx} ${cy - h} L ${cx} ${cy} Z`}
        fill={sh} opacity="0.55"
      />
      ...
      <path d={`M ${cx - 2} ${cy} L ${cx - 1.5} ${cy - 5} L ${cx + 1.5} ${cy - 5} L ${cx + 2} ${cy} Z`}
            fill={LMK.earthDark} stroke={ink} strokeWidth="0.35" />
      {/* simple painted band */}
      <path d={`M ${cx - w / 2 + 1.5} ${cy - 2} L ${cx + w / 2 - 1.5} ${cy - 2}`}
            stroke={LMK.rust} strokeWidth="0.5" opacity="0.6" />
      {smoke && (
...
// ── Horse — small profile, in corral ──
function Horse({ x, y, ink, color = "#5a3a20" }) {
  return (
    <g transform={`translate(${x} ${y})`}>
      {/* body */}
      <path d="M -7 -8 L -7 -10 L -5 -11 L 5 -11 L 7 -10 L 7 -8 L 5 -7 L -5 -7 Z"
            fill={color} stroke={ink} strokeWidth="0.4" />
      {/* neck + head */}
      <path d="M 5 -11 L 8 -14 L 10 -14 L 11 -12 L 9 -10 L 6 -9 Z"
            fill={color} stroke={ink} strokeWidth="0.4" />
      {/* mane */}
      <path d="M 5 -11 q 1 -2 3 -3" stroke={ink} strokeWidth="0.5" fill="none" />
      {/* legs */}
      <line x1="-5" y1="-7" x2="-5" y2="0" stroke={ink} strokeWidth="0.7" />
      <line x1="-2" y1="-7" x2="-2" y2="0" stroke={ink} strokeWidth="0.7" />
      <line x1="2"  y1="-7" x2="2"  y2="0" stroke={ink} strokeWidth="0.7" />
      <line x1="5"  y1="-7" x2="5"  y2="0" stroke={ink} strokeWidth="0.7" />
      {/* tail */}
      <path d="M -7 -9 q -2 1 -2 4" stroke={ink} strokeWidth="0.7" fill="none" />
    </g>
  );
}

// ── Sage brush — small low desert shrub ──
function SageBrush({ x, y, ink }) {
  return (
    <g transform={`translate(${x} ${y})`}>
...
// ── Parked weathered wagon — trader's, dusty ──
function ParkedWagon({ ink }) {
  return (
    <g>
      <path d="M -14 -8 C -16 -19, 16 -19, 14 -8 Z"
            fill="#d8c8a0" stroke={ink} strokeWidth="0.5" />
      {/* canvas patches */}
      <path d="M -4 -16 q 4 -1 6 1 l -1 3 l -5 -1 z" fill="#b89868" opacity="0.7" stroke={ink} strokeWidth="0.3" />
      <rect x="-14" y="-8" width="28" height="5" fill={LMK.earthDark} stroke={ink} strokeWidth="0.45" />
      <circle cx="-9" cy="-1" r="3.5" fill="none" stroke={ink} strokeWidth="0.6" />
      <circle cx="9" cy="-1" r="3.5" fill="none" stroke={ink} strokeWidth="0.6" />
...
Object.assign(window, { FortBridgerArt });
