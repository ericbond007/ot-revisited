...
<style>
  .status {
    ...
    /* Redesign: paper-card frame (frame only — the painted scene art inside
       is untouched). Double rule + soft drop, matching every other panel. */
    border: 3px double var(--of-ink-soft, #5a3f1c);
    border-radius: var(--of-r-sm, 3px);
    background: var(--of-paper-soft, #f0e0bd);
    box-shadow:
      inset 0 0 0 1px rgba(255, 245, 220, 0.30),
      0 1px 0 rgba(0, 0, 0, 0.06),
      0 4px 10px rgba(74, 46, 21, 0.14);
    gap: 0.4em;
  }
  ...
  .landscape {
    ...
    overflow: hidden;
    border-radius: var(--of-r-xs, 2px);
    /* Ink keyline + inset recess so the painted scene reads as set into
       the paper, like a tipped-in plate in a broadsheet. */
    border: 2px solid var(--of-ink-soft, #5a3f1c);
    box-shadow: inset 0 0 12px rgba(42, 29, 12, 0.30);
  }
